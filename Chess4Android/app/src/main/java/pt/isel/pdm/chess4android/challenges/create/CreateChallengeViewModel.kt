...
class CreateChallengeViewModel(app: Application) : AndroidViewModel(app) {
    ...
    fun createChallenge(name: String, message: String) {
        getApplication<PuzzleOfDayApplication>().challengesRepository.publishChallenge(
            name = name,
            message = message,
            onComplete = {
                _created.value = it
    ...
    fun removeChallenge() {
        ...
        val repo = getApplication<PuzzleOfDayApplication>().challengesRepository
    ...
    private fun waitForAcceptance(challengeInfo: ChallengeInfo) {
        subscription = getApplication<PuzzleOfDayApplication>().challengesRepository.subscribeToChallengeAcceptance(
